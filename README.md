...
開源的 SFTP、FTP、WebDAV、S3 和 SCP 客戶端。支援圖形介面和命令列操作，可以使用「命令」>「持續監控」功能，在本機檔案有更改時自動同步到遠端主機。雖然有即時同步功能，但個人覺得在自動化和排程同步方面不如 GoodSync 靈活好用。適合偶爾需要手動傳輸檔案或簡單同步的場景。

### 進程監控工具

#### 工作管理員 (Task Manager)

Windows 內建的系統監控工具，按 `Ctrl+Shift+Esc` 或 `Ctrl+Alt+Del` 快速開啟。當快捷鍵無法使用時（如系統卡頓或遠端桌面），可以在命令列輸入 `taskmgr` 開啟，或在工作列右鍵選擇「工作管理員」。可以查看所有執行中的進程、CPU、記憶體、磁碟和網路使用率。在「詳細資料」頁籤可以看到每個進程的詳細資訊，「效能」頁籤提供系統資源的即時監控圖表。對於一般的進程管理和系統監控已經相當足夠。

#### [LockHunter](https://lockhunter.com/)

當你嘗試刪除或移動檔案時遇到「檔案正在使用中」的錯誤，LockHunter 可以幫你找出是哪個進程佔用了該檔案。它能顯示所有鎖定檔案的進程，並提供解鎖、刪除進程或將檔案移至資源回收筒等選項。特別適合處理無法刪除的頑固檔案，支援整合到右鍵選單快速查看。
